...
def extract_csv_response(response_text):
    # ';'로 구분된 CSV 형식의 문자열을 StringIO 객체로 읽어들입니다.
    csv_data = StringIO(response_text)

    # 판다스의 read_csv 함수를 사용하여 데이터프레임으로 변환합니다.
...
def send_message(message_log):
    ...
    response = openai.ChatCompletion.create(
        ...
        temperature=0.1,  # 생성된 텍스트의 다양성을 조절하는 매개변수입니다.
    )
